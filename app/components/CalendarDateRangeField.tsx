...
function cleanRange(defaultFromValue: string, defaultToValue: string) {
  const from = cleanDate(defaultFromValue);
  const to = cleanDate(defaultToValue);
...
function formatDisplayRange(from: string, to: string, placeholder: string) {
  if (from && to) {
    return `${formatSingleDate(from)} - ${formatSingleDate(to)}`;
  }
  if (from) return `${formatSingleDate(from)} - Select end date`;
  return placeholder;
}

export default function CalendarDateRangeField({
  id,
  fromName,
  toName,
  defaultFromValue,
  ...
  const initialRange = cleanRange(defaultFromValue, defaultToValue);
  const [from, setFrom] = useState(initialRange.from);
  const [to, setTo] = useState(initialRange.to);
  const [viewMonth, setViewMonth] = useState(() =>
    getInitialMonth(initialRange.from),
  );
  ...
  const selectCurrentDate = () => {
    const dateValue = formatBookingDate(new Date());
    if (isDateDisabled(dateValue)) return;
    setFrom(dateValue);
    setTo(dateValue);
    setOpen(false);
  ...
  return (
    <div ref={wrapRef} className={wrapperClassName}>
      <input
        type="hidden"
        name={fromName}
        value={rangeComplete ? from : ""}
      />
      <input type="hidden" name={toName} value={rangeComplete ? to : ""} />
      <button
        id={id}
        type="button"
      ...
      >
        ...
        <span className={from ? "text-ink" : "text-ink/40"}>
          {formatDisplayRange(from, to, placeholder)}
        </span>
      </button>
        ...
        >
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            ...
            {cells.map((cell) => {
              ...
              const selected = dateValue === from || dateValue === to;
              const inRange = from && to && dateValue > from && dateValue < to;
