...
export default function AdminDateField({
  ...
  const selectDate = (date: Date) => {
    setValue(formatBookingDate(date));
    setOpen(false);
  };
  ...
  return (
    <div ref={wrapRef} className="relative mt-1">
      ...
      <button
        ...
        className="flex h-10 w-full items-center gap-2 rounded-xl border border-black/10 bg-white px-3 text-left text-sm text-ink shadow-[inset_0_1px_0_rgba(255,255,255,0.8),0_1px_2px_rgba(0,0,0,0.04)] outline-none transition-colors hover:bg-white/90 focus:border-brand focus:ring-2 focus:ring-brand/15"
      >
        ...
        >
          ...
          <div className="grid grid-cols-7 gap-1 text-center">
            {WEEKDAYS.map((day, index) => (
              <div key={`${day}-${index}`} className="pb-1 text-[10px] font-medium text-ink/35">
                {day}
              </div>
            ))}
            {cells.map((cell) => {
              ...
              return (
                <button
                  key={formatBookingDate(cell.date)}
                  type="button"
